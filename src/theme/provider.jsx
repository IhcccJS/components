...
/** @deprecated 使用 getChangeTheme 替代 */
...
export const getChangeTheme = function (getSetting) {
  ...
  const changeThemeTransitional = async (theme, event = null) => {
    //在不支持的浏览器里不做动画
    ...
    const keyframes = [{ clipPath: `circle(0px at ${x}px ${y}px)` }, { clipPath: `circle(${endRadius}px at ${x}px ${y}px)` }];
    //开始动画
...
function ThemeProvider({ theme, list, toggle, onChange, updateTheme, children }) {
  ...
  React.useEffect(() => {
    changeThemeInternal(theme, mouseEventRef.current);
  ...
  return (
    <ThemeContext.Provider value={{ selected: theme, selectedIndex, darkMode, list, toggle, setTheme }}>{children}</ThemeContext.Provider>
  );
}
