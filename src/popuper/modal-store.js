...
class ModalStore {
  ...
  getKey(name, original) {
    ...
    const keys = isKey ? name.split('/') : [this.namespace].concat(name).filter(isFilterKey);
  ...
  getRenderModal() {
    ...
    this.modalList.forEach((item, key) => {
      ...
      const renderDom = React.createElement(
        ...
        content,
        // React.createElement(React.Suspense, { fallback: 'loading...' }, content),
      );
